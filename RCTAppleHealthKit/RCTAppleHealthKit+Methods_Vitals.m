...
- (void)vitals_getHeartRateSamples:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    if (includeManuallyAdded == false) {
        NSPredicate *manuallyAddedPredicate = [NSPredicate predicateWithFormat:@"metadata.%K != YES", HKMetadataKeyWasUserEntered];
        predicate = [NSCompoundPredicate andPredicateWithSubpredicates:@[predicate, manuallyAddedPredicate]];
    }
...
{
    HKQuantityType *aFibBurdenType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierAtrialFibrillationBurden];
    ...
    NSPredicate * predicate = [RCTAppleHealthKit predicateForSamplesBetweenDates:startDate endDate:endDate];
    [self fetchQuantitySamplesOfType:aFibBurdenType
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting AFib burden samples: %@", error);
                                  callback(@[RCTMakeError(@"error getting AFib burden samples", nil, nil)]);
                                  return;
                              }
                          }];
}
...
{
    HKQuantityType *quantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierHeartRateVariabilitySDNN];
...
{
    HKQuantityType *quantityType = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierRestingHeartRate];
    HKUnit *count = [HKUnit countUnit];
    HKUnit *minute = [HKUnit minuteUnit];
    HKUnit *unit = [count unitDividedByUnit:minute];
    [self saveQuantitySamples:samples
